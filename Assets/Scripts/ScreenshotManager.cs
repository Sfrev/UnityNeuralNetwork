...
public class ScreenshotManager : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        if (screenshotCamera.gameObject.activeInHierarchy)
        {
            ...
            while (System.IO.File.Exists(string.Format("{0}/Screenshots/CameraScreenshot {1}.png", Application.dataPath, screenshotIndex)))
            {
                screenshotIndex++;
            }

            string path = string.Format("{0}/Screenshots/CameraScreenshot {1}.png", Application.dataPath, screenshotIndex);
            System.IO.File.WriteAllBytes(path, byteArray);

            Debug.Log(string.Format("Saved CameraScreenshot {0}.png", screenshotIndex));
            screenshotCamera.gameObject.SetActive(false);
        }
    }
